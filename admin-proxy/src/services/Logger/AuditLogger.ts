...
class AuditLogger extends Logger {
  ...
  constructor(logGroup: string, additionalAttributes:string[], retentionPolicy: number = 30,) {
  ...
  public logRequest(status: number, details:any, req: Request) {
    ...
    try {
      const additionalInfo = this.retrieveAdditionalInfo(JSON.parse(details || '{}'));
      if (status === 200) {
      ...
      this.log('error', message, JSON.stringify({
